...
int positionKFromAngle(unsigned char arm_id, float angle)
{
	...
	if (MXID1 == arm_id) {
		IDPOSK = ID1POSK;
		//IDAngle = ID1Angle;
		rate = REDUCTION_RATE1;
		temp = ID1Angle - angle;
	}
	else if(MXID2 == arm_id) {
		IDPOSK = ID2POSK;
		//IDAngle = ID2Angle;
		rate = REDUCTION_RATE2;
		temp = angle - ID2Angle;
	}
	else if(MXID3 == arm_id) {
		IDPOSK = ID3POSK;
		//IDAngle = ID3Angle;
		rate = REDUCTION_RATE3;
		temp = angle - ID3Angle;
	}
	else {
		IDPOSKY = -1;
...
/***********************************************
函数原型：
	int kFromRadianSpeed(int arm_id, float radianSpeed)
参数：
	id:指定的舵机的ID
...
int kFromRadianSpeed(int arm_id, float radianSpeed)
{
	...
	kSpeed = (int)(vec /SpeedUnit + 0.35);	//把rpm转化为舵机速度寄存器刻度
